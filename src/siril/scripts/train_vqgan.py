...
from siril.models.codec_models.vqgan.discriminator import Discriminator
from siril.models.codec_models.vqgan.lpips import LPIPS
from siril.models.codec_models.vqgan.utils import load_data, weights_init
from siril.models.codec_models.vqgan.vqgan import VQGAN
        ...
        for epoch in range(args.epochs):
            ...
            with tqdm(range(len(train_dataset))) as pbar:
                ...
                for i, imgs in zip(pbar, train_dataset):
                    ...
                    # if i % 10 == 0:
                    latantcode=self.vqgan.getEncode_image_visual()
                    # print("latantcode>>>>",latantcode.shape)
                    ...
                    with torch.no_grad():
                        ...
                        vutils.save_image(latantcode.sum(dim=1, keepdim=True)[:4], os.path.join(f"{self.work_dir}/visualization/epoch{epoch}", f"{i}_codez.jpg"), nrow=4)

                    pbar.set_postfix(
                        VQ_Loss=np.round(vq_loss.cpu().detach().numpy().item(), 5),
                        GAN_Loss=np.round(gan_loss.cpu().detach().numpy().item(), 3)
                    )
                    vqloss.append(np.round(vq_loss.cpu().detach().numpy().item(), 5))
                    ganloss.append(np.round(gan_loss.cpu().detach().numpy().item(), 3))
                    pbar.update(0)
                vqloss_mean=np.array(vqloss).mean()
                ganloss_mean=np.array(ganloss).mean()
                val_loss = self.eval(val_dataset, epoch*steps_per_epoch+i)
                is_best = val_loss < best_loss
                best_loss = min(best_loss, val_loss)
                with self.logger.log_and_dump_ctx(epoch, ty='train') as log:
                    log('trainganloss',ganloss_mean)
                    log('trainvqloss_mean', vqloss_mean)
...
@hydra.main(config_path="../siril/configs/models/codec_models/vqgan", config_name="default")
def main(cfg):
    TrainVQGAN(cfg)
