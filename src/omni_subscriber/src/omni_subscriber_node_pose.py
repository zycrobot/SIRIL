...
def pose_callback_left(data):
    ...
    arm.gotoPositionInmicrometers(x0+(x-x1),y0+(y-y1),z0+(z-z1))
...
def pose_callback_right(data):
    ...
    arm.gotoPositionInmicrometers(y0-(y-y1),x0+(x-x1),z0-(z-z1))
...
def subscriber_node():
    ...
    if 'left' in pose:
        ...
        rospy.Subscriber(pose, PoseStamped, pose_callback_left,queue_size=1)
    else:
        rospy.loginfo('pose_callback_right')
        rospy.Subscriber(pose, PoseStamped, pose_callback_right,queue_size=1)
    rospy.spin()
