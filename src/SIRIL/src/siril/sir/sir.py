...
from ..Transformer.transformer import VideoGPTTransformer
...
class SIR(nn.Module):
    ...
    def calc_loss(self, imgs):
        batch_embs, batch_indices = self.imgs_to_batch(imgs, self.reward_type)
        sos_tokens = self.model.calc_sos_tokens(imgs, batch_embs).tile((batch_embs.shape[0], 1, 1))

        loss = -1.0*self.cal_log_prob(batch_embs, batch_indices, sos_tokens, target_indices=batch_indices, reward_type=self.reward_type)
        return loss.mean()  
