...
class VideoGPTTransformer(nn.Module):
    def __init__(self, args):
        ...
        self.action_mlp = nn.Sequential(
            nn.Linear(args.latent_size * args.code_dim + args.action_dim, args.mlp_hidden_size),
            nn.ReLU(),
            nn.Linear(args.mlp_hidden_size, args.action_dim)
        )
    ...
    def output(self, x, past_actions=None, compute_joint=True):
            embs, indices = self.encode_to_z(x)
            target = indices
            sos_tokens = self.calc_sos_tokens(x, embs)

            if not self.use_vqemb:
                # Index-based input handling
                mask = torch.bernoulli(self.pkeep * torch.ones(indices.shape, device=indices.device))
                mask = mask.round().to(dtype=torch.int64)
                random_indices = torch.randint_like(indices, self.transformer.config.vocab_size)
                new_indices = mask * indices + (1 - mask) * random_indices
                ...
                # Embedding-based input handling
                mask = torch.bernoulli(self.pkeep * torch.ones(indices.shape, device=indices.device))
                mask = mask.round().to(dtype=torch.int64)[:, :, None]
                new_embs = embs * mask
                new_embs = torch.cat([sos_tokens, new_embs], dim=1)
            ...
            # Concatenate past actions with current embeddings for MLP
            if past_actions is not None:
                # Flatten the embeddings and concatenate with past actions
                flattened_embs = embs.reshape(embs.size(0), -1)
                action_input = torch.cat((flattened_embs, past_actions), dim=-1)
                action_predictions = self.action_mlp(action_input)
            else:
                action_predictions = None

            if compute_joint:
                return logits, target, action_predictions
            else:
                num_valid_logits = int(logits.shape[1] / x.shape[1])
                return logits[:, -num_valid_logits:], target[:, -num_valid_logits:], action_predictions
